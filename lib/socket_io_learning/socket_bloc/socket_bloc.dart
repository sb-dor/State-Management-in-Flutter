...
sealed class SocketState with _$SocketState {
  ...
  const factory SocketState.completed({required User toUser, required List<String> messages}) =
...
class SocketBloc extends Bloc<SocketEvent, SocketState> {
  ...
  void _init(_InitialEventOnSocketEvent event, Emitter<SocketState> emit) async {
  ...
  void _joinToTheRoom(_JoinToTheRoomEventOnSocketEvent event, Emitter<SocketState> emit) async {
    _socketClientService.joinToRoom("room_${event.user.id}");
    emit(SocketState.completed(toUser: event.user, messages: <String>[]));
  }

  void _leaveRoom(_LeaveRoomEventOnSocketEvent event, Emitter<SocketState> emit) async {
  ...
  void _sendMessage(_SendMessageEvemtOnSocketEvent event, Emitter<SocketState> emit) async {
    if (state is! CompletedStateOnSocketStates) return;
    final currentState = state as CompletedStateOnSocketStates;
  ...
  void _handleMessages(_HandleMessagesEventOnSocketEvent event, Emitter<SocketState> emit) async {
    if (state is! CompletedStateOnSocketStates) return;
